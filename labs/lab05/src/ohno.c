...
ohno(const char* message, ohno_severity_t severity)
{
    switch (severity) {
        case OHNO_WARNING:
            fprintf(state->out, "WARNING #%d in %s %s", state->error_number, state->name, message);
            break;
        case OHNO_SERIOUS:
            fprintf(state->out, "SERIOUS #%d in %s %s", state->error_number, state->name, message);
            break;
        case OHNO_FATAL:
            fprintf(state->out, "FATAL #%d in %s %s", state->error_number, state->name, message);
            break;
