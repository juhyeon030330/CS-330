...
string Cipher::encrypt(string raw)
{
    ...
    for(char& c : raw) {
        if(c == ' ') continue;
        unsigned int pos = find_pos("abcdefghijklmnopqrstuvwxyz", LOWER_CASE(c));

        if(c > 'Z') {
            c = smile->cipher_alpha[pos];
        } else {
            c = UPPER_CASE(smile->cipher_alpha[pos]);
        }
...
string Cipher::decrypt(string enc)
{
    ...
    for(char& c : enc) {
        if(c == ' ') continue;
        unsigned int pos = find_pos(smile->cipher_alpha, LOWER_CASE(c));
